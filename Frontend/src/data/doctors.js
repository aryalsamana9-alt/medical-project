/**
 * 12 Medical/Mental Health Specialists
 * Each profile includes: id, name, specialty, initials, bio, rating, isAvailable,
 * keywords (for matching), and replies (8+ professional, discipline-specific responses).
 */
...
const doctors = [
  {
    ...
    rating: 4.8,
    isAvailable: true,
    keywords: ['surgery', 'surgical', 'operation', 'appendicitis', 'gallbladder', 'hernia', 'laparoscopic', 'biopsy', 'tumor', 'wound', 'abdominal'],
    replies: [
  ...
  },
  {
    ...
    rating: 4.9,
    isAvailable: true,
    keywords: ['bone', 'joint', 'knee', 'shoulder', 'hip', 'fracture', 'sprain', 'ligament', 'ACL', 'arthritis', 'back pain', 'spine', 'sports injury', 'physical therapy'],
    replies: [
  ...
  },
  {
    ...
    rating: 4.7,
    isAvailable: true,
    keywords: ['headache', 'migraine', 'seizure', 'epilepsy', 'tremor', 'Parkinson', 'Alzheimer', 'memory', 'dementia', 'numbness', 'tingling', 'dizziness', 'vertigo', 'stroke', 'nerve', 'brain'],
    replies: [
  ...
  },
  {
    ...
    rating: 4.9,
    isAvailable: true,
    keywords: ['depression', 'anxiety', 'panic', 'PTSD', 'trauma', 'bipolar', 'mood', 'stress', 'therapy', 'mental health', 'insomnia', 'OCD', 'ADHD', 'grief', 'suicidal', 'counseling'],
    replies: [
  ...
  },
  {
    ...
    rating: 4.8,
    isAvailable: true,
    keywords: ['heart', 'chest pain', 'palpitations', 'arrhythmia', 'blood pressure', 'hypertension', 'cholesterol', 'cardiac', 'heart attack', 'angina', 'shortness of breath', 'cardiovascular'],
    replies: [
  ...
  },
  {
    ...
    rating: 4.9,
    isAvailable: true,
    keywords: ['child', 'children', 'baby', 'infant', 'pediatric', 'vaccination', 'immunization', 'growth', 'development', 'ADHD', 'asthma', 'allergies', 'fever', 'rash'],
    replies: [
  ...
  },
  {
    ...
    rating: 4.7,
    isAvailable: true,
    keywords: ['skin', 'rash', 'acne', 'eczema', 'psoriasis', 'dermatitis', 'mole', 'melanoma', 'skin cancer', 'sunburn', 'sunscreen', 'rosacea', 'itching', 'hair loss'],
    replies: [
  ...
  },
  {
    ...
    rating: 4.8,
    isAvailable: true,
    keywords: ['pregnancy', 'prenatal', 'gynecology', 'menstrual', 'period', 'cramps', 'pelvic pain', 'endometriosis', 'PCOS', 'menopause', 'fertility', 'contraception', 'women'],
    replies: [
      ...
      "UTI symptoms like burning and frequency should be treated promptly to prevent the infection from ascending to the kidneys. Increasing water intake may help prevent recurrence.",
    ],
  },
  {
    id: 'doctor-9',
    ...
    rating: 4.9,
    isAvailable: false,
    keywords: ['cancer', 'tumor', 'chemotherapy', 'radiation', 'oncology', 'lump', 'breast cancer', 'lung cancer', 'leukemia', 'lymphoma', 'biopsy', 'remission', 'immunotherapy'],
    replies: [
      ...
      "Colonoscopy is the gold standard for colorectal cancer screening. Finding and removing precancerous polyps during the procedure prevents cancer.",
  ...
  },
  {
    ...
    rating: 4.6,
    isAvailable: true,
    keywords: ['lung', 'breathing', 'cough', 'asthma', 'COPD', 'bronchitis', 'pneumonia', 'shortness of breath', 'wheezing', 'oxygen', 'sleep apnea', 'CPAP', 'pulmonary'],
    replies: [
  ...
  },
  {
    ...
    rating: 4.7,
    isAvailable: true,
    keywords: ['stomach', 'abdominal', 'digestive', 'IBS', 'IBD', 'Crohn', 'colitis', 'acid reflux', 'GERD', 'heartburn', 'nausea', 'diarrhea', 'constipation', 'bloating', 'liver', 'gut'],
    replies: [
      "Chronic bloating and abdominal discomfort could indicate IBS, food intolerances, or SIBO. A detailed food and symptom diary for 2 weeks is a great starting point.",
      ...
      "Colonoscopy is the most effective tool we have for colorectal cancer prevention. For average-risk individuals, screening begins at age 45.",
      ...
      "The gut-brain axis is fascinating — stress and anxiety can directly trigger digestive symptoms. Mind-body approaches like CBT can be as important as medication.",
      ...
      "Constipation often responds to increased fiber, water intake, and regular exercise. If these don't help, we'll investigate for underlying causes like hypothyroidism.",
  ...
  },
  {
    ...
    rating: 4.8,
    isAvailable: true,
    keywords: ['diabetes', 'thyroid', 'hormone', 'metabolism', 'weight', 'obesity', 'insulin', 'blood sugar', 'glucose', 'fatigue', 'hot flashes', 'thirst', 'endocrine', 'osteoporosis'],
    replies: [
...
/**
 * Calculate match score for a doctor based on user symptom keywords.
 * 2x weight modifier for matching doctor specialty keywords.
 */
export function calculateMatchScore(doctor, userKeywords) {
  if (!userKeywords || userKeywords.length === 0) return 0;
  const doctorKw = doctor.keywords.map((k) => k.toLowerCase());
  let matches = 0;
  let totalWeight = 0;
  for (const word of userKeywords) {
    const w = word.toLowerCase();
    totalWeight += 1;
    if (doctorKw.some((dk) => dk.includes(w) || w.includes(dk))) {
      matches += 2;
    }
  }
  return Math.min(100, Math.round((matches / (totalWeight * 2)) * 100));
}

export function getTopMatches(userKeywords, limit = 3) {
  const scored = doctors
    .map((doc) => ({ doctor: doc, score: calculateMatchScore(doc, userKeywords) }))
